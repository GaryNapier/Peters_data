...
Data <- "t1" # EITHER "t1" or "t2"

Analyses_nos <- c(45, 47) # NB : MUST BE > 10
...
---
title: "AR1 analyses of Peters `r Data` data"
author: "Gary Napier"
...
Group <- c(rep("Ctrl", length(Ctrl)), rep("Pat", length(Pat)), rep("Psych ctrl", length(Psych_ctrl)))
...
Priors_names <- c("NuPrimeMu", "NuPrimeSa", "KappaMu", "KappaSa", "Om2Mu", 
            "Om2Sa", "ThetaMu", "ThetaSa", "m2Mu", "m2Sa", 
            "Phi2Mu", "Phi2Sa", "SA2Mu", "Sa2Sa")
...
Sa2_vector <- vector("list", length(Analyses[[1]]))
...
Sa2_vector[[Analyses_nos[j]]][i] <- 
Analyses[[1]][[Analyses_nos[j]]][[1]][[1]][[i]][[1]][9][[1]][2][[1]][,2]
...
# Create ANOVA models:
# Sa2 
Aov_mod_sa2 <- lapply(Sa2_vector, function(x) aov(x$Sa2_vector ~ x$Group))
# Get post-hoc tests
Post_hoc_sa2 <- lapply_mod(Aov_mod_sa2, function(i) 
  data.frame(TukeyHSD(Aov_mod_sa2[[i]])$`x$Group`))
...
# Sa2
for (i in 1: length(Post_hoc_sa2)){
  Post_hoc_sa2[[i]]$Comparison <- factor(row.names(Post_hoc_sa2[[i]]))
}
...
# Labels <- factor(c(rep("Sa2", N_groups), rep("Omega2", N_groups), rep("Omega3", N_groups),  rep("Nu", N_groups), rep("M2", N_groups), rep("M3", N_groups), rep("Phi2", N_groups), rep("Phi3", N_groups)), levels = c("Sa2", "Omega2", "Omega3", "Nu", "M2", "M3", "Phi2", "Phi3")) 
Labels <- factor(c(rep("Sa2", N_groups), 
                   rep("Omega2", N_groups), 
                   rep("Nu", N_groups), 
                   rep("M2", N_groups), 
                   rep("Phi2", N_groups)), 
...
# }
# for (i in 1:length(Analyses_nos)){
# Post_hoc_df[[i]] <- cbind(rbind(Post_hoc_sa2[[i]], Post_hoc_om2[[i]], Post_hoc_om3[[i]], Post_hoc_nu[[i]], Post_hoc_M2[[i]], Post_hoc_M3[[i]], Post_hoc_phi2[[i]], Post_hoc_phi3[[i]]), Labels)
# }
for (i in 1:length(Analyses_nos)){
Post_hoc_df[[i]] <- cbind(rbind(Post_hoc_sa2[[i]], Post_hoc_om2[[i]], Post_hoc_nu[[i]], Post_hoc_M2[[i]], Post_hoc_phi2[[i]]), Labels)
}
...
```{r echo = FALSE, cache=F, htmlTable, results = 'asis'}
  ...
  # #Aov_mod_sa2
  # SW_sa2 <- signif(sapply(Aov_mod_sa2, function(x) Shapiro_NA(residuals(x))), digits = 3 )
  # 
  # # Aov_mod_om2
  ...
  # # SW_data <- data.frame(Model_names, SW_sa2, SW_omega2, SW_omega3, SW_nu, SW_M2, SW_M3, SW_phi2, SW_phi3)
  #   SW_data <- data.frame(Model_names, SW_sa2, SW_omega2, SW_nu, SW_M2, SW_phi2)
  #   
  # do.call("htmlTable", c(list(txtRound(SW_data, 2)), Table_options))
...
```{r echo = FALSE, cache=F, results = 'asis'}
  ...
  # # Sa2
  # BF_sa2 <- signif(sapply(Sa2_vector, function(x) leveneTest(Sa2_vector ~ Group, data=x,  center=median)$P), digits=3)
  # 
  # # Om2
  ...
  # 
  # # Clean:
  # BF_sa2 <- BF_sa2[!is.na(BF_sa2) | is.nan(BF_sa2)]
  ...
  # # BF_data <- data.frame(Model_names, BF_sa2, BF_omega2, BF_omega3, BF_nu, BF_M2, BF_M3, BF_phi2, BF_phi3)
  #   BF_data <- data.frame(Model_names, BF_sa2, BF_omega2, BF_nu, BF_M2, BF_phi2)
  # 
...
```{r echo = FALSE, results = 'asis', warning=FALSE, message=FALSE, cache=F}
    ...
    # SA2
    # Mean / sd groups
        Sa2_mean_sd <- do.call("rbind", lapply( lapply_mod(Aov_mod_sa2, function(i){
     data.frame(signif(rbind(
          c(mean(Sa2_vector[[i]]$Sa2_vector[Ctrl]), sd(Sa2_vector[[i]]$Sa2_vector[Ctrl])), 
          c(mean(Sa2_vector[[i]]$Sa2_vector[Pat]), sd(Sa2_vector[[i]]$Sa2_vector[Pat])),  
          c(mean(Sa2_vector[[i]]$Sa2_vector[Psych_ctrl]), sd(Sa2_vector[[i]]$Sa2_vector[Psych_ctrl])) 
...
```{r echo = FALSE, results = 'asis', warning=FALSE, message=FALSE, cache=F}
  ...
  # Sa2
  Sa2_anova <- lapply_mod(Aov_mod_sa2, function(i){summary(Aov_mod_sa2[[i]])})
  Sa2_anova_df <- do.call("rbind", sapply(Sa2_anova, function(x) lapply(x, data.frame)))
  Df <- unique(Sa2_anova_df$Df)
  Col_names <- c(sprintf("F(%g,%g)",Df[1], Df[2]), "p")
  Sa2_anova_df <- dplyr::select(Sa2_anova_df, -(Df:Mean.Sq))
  Sa2_anova_df <- Sa2_anova_df[-c(seq(2, nrow(Sa2_anova_df), 2)),]
  colnames(Sa2_anova_df) <- Col_names
  rownames(Sa2_anova_df) <- Model_names
  Sa2_anova_df <- signif(Sa2_anova_df, digits = 2)
  ...
  # Anova_table <- cbind(Om2_anova_df, Om3_anova_df, Nu_anova_df, M2_anova_df, M3_anova_df, Phi2_anova_df, Phi3_anova_df)
    # Anova_table <- cbind(Sa2_anova_df, Om2_anova_df, Om3_anova_df, Nu_anova_df, M2_anova_df, M3_anova_df, Phi2_anova_df, Phi3_anova_df)
    Anova_table <- cbind(Sa2_anova_df, Om2_anova_df, Nu_anova_df, M2_anova_df, Phi2_anova_df)
...
```{r echo = FALSE, results = 'asis', warning=FALSE, message=FALSE, cache=F}
  ...
  Comparisons <- rownames(Post_hoc_om2[[1]])
    ...
    # Sa2
    Post_hoc_sa2 <- do.call("rbind", Post_hoc_sa2)
    rownames(Post_hoc_sa2) <- NULL
    Post_hoc_sa2 <- dplyr::select(Post_hoc_sa2, p.adj)
    ...
    # Post_hoc_table <- cbind(Post_hoc_sa2, Post_hoc_om2, Post_hoc_om3, Post_hoc_nu, Post_hoc_M2, Post_hoc_M3, Post_hoc_phi2, Post_hoc_phi3)
    Post_hoc_table <- cbind(Post_hoc_sa2, Post_hoc_om2, Post_hoc_nu, Post_hoc_M2, Post_hoc_phi2)
...
# Effect_size <- c(Aov_mod_sa2, Aov_mod_om2, Aov_mod_om3, Aov_mod_nu, Aov_mod_M2, Aov_mod_M3, Aov_mod_phi2, Aov_mod_phi3)
Effect_size <- c(Aov_mod_sa2, Aov_mod_om2, Aov_mod_nu, Aov_mod_M2, Aov_mod_phi2)
...
# # Sa2 K-W
#     Sa2_KW <- lapply_mod(Sa2_vector, function(i)tryCatch({dunn.test(Sa2_vector[[i]]$Sa2_vector, 
#                                       g = Sa2_vector[[i]]$Group, 
...
# # Chi_sq <- signif(unlist(mapply(function(x) {x[[1]]}, x = c(Sa2_KW, Om2_KW, Om3_KW, Nu_KW, M2_KW, M3_KW, Phi2_KW, Phi3_KW) ) ), digits=3)
# Chi_sq <- signif(unlist(mapply(function(x) 
#   {x[[1]]}, 
#   x = c(Sa2_KW, Om2_KW, Nu_KW, M2_KW, Phi2_KW) ) ), digits=3)
# 
...
# All_KW <- c(Sa2_KW, Om2_KW, Om3_KW, Nu_KW, M2_KW, M3_KW, Phi2_KW, Phi3_KW)
# All_KW <- c(Sa2_KW, Om2_KW, Nu_KW, M2_KW, Phi2_KW)
# 
...
for (i in 1:length(Analyses_nos)){
  ...
  grid.arrange(Sa2_box+theme(legend.position="none"), ### Sa2
...
for (i in 1:length(Analyses_nos)){
  ...
  Sa2_dens <- ggplot(Sa2_vector[[i]], aes(Sa2_vector))+
    ...
    scale_x_continuous(limits = Y_limits, breaks = Y_breaks)
